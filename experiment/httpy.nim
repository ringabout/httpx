...
import net, nativesockets, os, httpcore, strutils
                  ...
                  if not fut.isNil:
                    fut.callback =
                      proc (arg: pointer = nil) {.gcsafe.} =
                        onRequestFutureComplete(cast[Future[void]](arg), selector, fd)
                        validateResponse()
                  else:
                    validateResponse()
...
proc updateDate(arg: pointer = nil) {.gcsafe.} =
...
proc eventLoop(params: (OnRequest, Settings)) =
  ...
  updateDate(cast[pointer](0))
  chronos.addTimer(1000.int64, updateDate)
